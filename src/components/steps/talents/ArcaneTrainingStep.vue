<template>
    <StepFrame>
        ...
        <Card glass class="added-theorems-card">
            <ul v-if="adventurer.talentsData[Step.ARCANE_TRAINING].theorems.length">
                <Card
                    v-for="(theorem, index) in adventurer.talentsData[Step.ARCANE_TRAINING].theorems"
                    class="added-theorem"
                >
                    <div>
        ...
        <Card class="theorem-creation">
            ...
            <div class="theorem-inputs">
                <InputGroup
                    v-model="adventurer.talentsData[Step.ARCANE_TRAINING].builder.theorem"
                    :placeholder="t('Step.Spellcraft.Theorems.Name.placeholder')"
                >
                    {{ t('Step.Spellcraft.Theorems.Name.label') }}
                </InputGroup>
                <InputGroup
                    v-model="adventurer.talentsData[Step.ARCANE_TRAINING].builder.magicSchool"
            ...
            <div class="crucibles">
                <CrucibleCard
                    ...
                    v-model="adventurer.talentsData[Step.ARCANE_TRAINING].spellCrucibles.style"
                />
                <CrucibleCard
                    :title="t('Step.Spellcraft.Essence.label')"
                    :items="spellcraftData.essence"
                    :label-function="(item: string) => t(`Step.Spellcraft.Crucible.${item}`)"
                    v-model="adventurer.talentsData[Step.ARCANE_TRAINING].spellCrucibles.essence"
                />
                <CrucibleCard
                    :title="t('Step.Spellcraft.Form.label')"
                    :items="spellcraftData.form"
                    :label-function="(item: string) => t(`Step.Spellcraft.Crucible.${item}`)"
                    v-model="adventurer.talentsData[Step.ARCANE_TRAINING].spellCrucibles.form"
                />
            </div>
            ...
            <TableCard :title="t('Step.Spellcraft.Magic-schools.label')" :items="spellcraftData.magicSchools">
                <template #item="{ item }">
                    <div
                        class="school-item"
                        @click="
                            adventurer.talentsData[Step.ARCANE_TRAINING].builder.magicSchool = t(
...
const talent = talentDefinitionsData.find((talent) => talent.id === Step.ARCANE_TRAINING)!;

const currentTheorem = computed(() => {
    return {
        name: props.adventurer.talentsData[Step.ARCANE_TRAINING].builder.theorem,
        magicSchool: props.adventurer.talentsData[Step.ARCANE_TRAINING].builder.magicSchool
    };
});

const isValidTheorem = computed(() => {
    return (
        props.adventurer.talentsData[Step.ARCANE_TRAINING].builder.theorem &&
        props.adventurer.talentsData[Step.ARCANE_TRAINING].builder.magicSchool
    );
});
...
function onClickAddTheorem() {
    if (isValidTheorem.value) {
        props.adventurer.talentsData[Step.ARCANE_TRAINING].theorems.push(currentTheorem.value);
        props.adventurer.talentsData[Step.ARCANE_TRAINING].builder.theorem = '';
        props.adventurer.talentsData[Step.ARCANE_TRAINING].builder.magicSchool = '';
    }
}
...
function onClickRemoveTheorem(index: number) {
    if (index > -1) props.adventurer.talentsData[Step.ARCANE_TRAINING].theorems.splice(index, 1);
}
</script>
